        ...
        _data.read('tokens', id, (err, tokenData) => {
            if (!err && tokenData) {
                ...
                if (tokenData.expires > Date.now()) {
                    ...
                    callback(400, {Error: 'The token has already expired, and can\'t be update '});
                }
            } else {
            ...
            if (!err && tokenData) {
                ...
                _data.read('users', userPhone, (err, userData) => {
                    if (!err && userData) {
                        ...
                        // Verify that the user has less than the nomber of max-checks-per-user
                                ...
                                _data.read('users', checkData.userPhone, (err, userData) => {
                                    if (!err && userData) {
                                        ...
                                        if (checkPosition > -1) {
                                            ...
                                            callback(500, {Error: 'Could not find the check on the use\'s object'});
                                        }
                                    } else {
                                        callback(500, {Error: 'Could not find the user who create the check, so could not remove the check from the list of checks on the user object'});
                                    }
                                });
                            } else {
                                callback(400, {Error: 'Could not find the specified user'});
